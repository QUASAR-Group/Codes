...
void BLCMDprofile::Entry::UserZSteppingAction(const G4Track *track)
{
	...
	momentum[0]=atan((momentum[0])/momentum.mag());
	momentum[1]=atan((momentum[1])/momentum.mag());
	sumPx += w*momentum[0];
	sumPy += w*momentum[1];
	sumPz += w*momentum[2];
	M[0][0] += w*pos[0]*pos[0];
...
void BLCMDprofile::callback(int type)
{
	...
	if(printing) {
		...
		fprintf(fd,"#mm - mm mm mm mm mm-mrad mm-mrad mm-mrad m m m - - - MeV/c\n");
	}
	...
	for(unsigned i=0; i<entries.size(); ++i) {
		...
		G4double meanX = entries[i]->sumX;
		G4double meanY = entries[i]->sumY;
		G4double meanPtot = entries[i]->sumPtot;
		G4double meanPx = entries[i]->sumPx;
		G4double meanPy = entries[i]->sumPy;
		G4double meanPz = entries[i]->sumPz/sumW;
		G4double **M = entries[i]->M;
		M[0][0] =(sumW*M[0][0] - meanX*meanX)/((sumW-1)*sumW);
		M[0][1] = (sumW*M[0][1] - meanX*meanPx)/((sumW-1)*sumW);
		M[0][2] = (sumW*M[0][2] - meanX*meanY)/((sumW-1)*sumW);
		M[0][3] = (sumW*M[0][3] - meanX*meanPy)/((sumW-1)*sumW);
		M[1][0] = (sumW*M[1][0]- meanPx*meanX)/((sumW-1)*sumW);
		M[1][1] = (sumW*M[1][1] - meanPx*meanPx)/((sumW-1)*sumW);
		M[1][2] = (sumW*M[1][2] - meanPx*meanY)/((sumW-1)*sumW);
		M[1][3] = (sumW*M[1][3] - meanPx*meanPy)/((sumW-1)*sumW);
		M[2][0] = (sumW*M[2][0] - meanY*meanX)/((sumW-1)*sumW);
		M[2][1] = (sumW*M[2][1] - meanY*meanPx)/((sumW-1)*sumW);
		M[2][2] = (sumW*M[2][2] - meanY*meanY)/((sumW-1)*sumW);
		M[2][3] = (sumW*M[2][3] - meanY*meanPy)/((sumW-1)*sumW);
		M[3][0] = (sumW*M[3][0] - meanPy*meanX)/((sumW-1)*sumW);
		M[3][1] = (sumW*M[3][1] - meanPy*meanPx)/((sumW-1)*sumW);
		M[3][2] = (sumW*M[3][2] - meanPy*meanY)/((sumW-1)*sumW);
		M[3][3] = (sumW*M[3][3] - meanPy*meanPy)/((sumW-1)*sumW);
		G4double sigmaX = sqrt(M[0][0]);
		G4double sigmaY = sqrt(M[2][2]);
		G4double mass = pd->GetPDGMass();
		G4double emitX = sqrt(M[0][0]*M[1][1]-M[1][0]*M[0][1]);
		G4double emitY = sqrt(M[2][2]*M[3][3]-M[3][2]*M[2][3]);
		G4double emitTrans = sqrt(sqrt(determinant(M,4)));
		G4double alphaX = -M[0][1]/emitX;
		G4double alphaY = -M[2][3]/emitY;
		G4double alphaTrans = (alphaX+alphaY)/2.0;
		G4double betaX = 0.001*M[0][0]/emitX;
		G4double betaY= 0.001*M[2][2]/emitY;
		G4double betaTrans = (betaX+betaY)/2.0;
		fprintf(fd,"%.1f %d %.3f %.3f %.3f %.3f %.4g %.4g %.4g %.4g %.4g %.4g %.4g %.4g %.4g %.4g %.4g \n",
			entries[i]->z,n,meanX,sigmaX,meanY,sigmaY,emitX,emitY,emitTrans,
			betaX,betaY,betaTrans,alphaX,alphaY,alphaTrans,meanPtot/MeV, meanPx);
	}
	if(printing) {
		if(fd != stdout)
