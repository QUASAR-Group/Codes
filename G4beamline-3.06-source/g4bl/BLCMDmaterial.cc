...
BLCMDmaterial::BLCMDmaterial() : BLCommand()
{
	...
	// default values
	a = z = density = ionpotential = UNDEFINED;
...
int BLCMDmaterial::command(BLArgumentVector& argv, BLArgumentMap& namedArgs)
{
	...
	if(argv.size() > 1) {		// mixture of other materials
		...
		if(ionpotential == UNDEFINED) {
		ionpotential=material->GetIonisation()->GetMeanExcitationEnergy();
	}
		material->GetIonisation()->SetMeanExcitationEnergy( ionpotential ); //in MeV
		printf("material %-6s Z=%.2f A=%.2f\n",name.c_str(),z,
							a/(gram/mole));
		printf("                density=%.3f temperature=%.0f pressure=%.1f\n ionpotential=%.3f",
			density/(gram/cm3),temperature,pressure/atmosphere,ionpotential/eV);
...
void BLCMDmaterial::defineNamedArgs()
{
	...
	argDouble(ionpotential,"ionpotential","Ionisation potential of the material (MeV)",MeV);
