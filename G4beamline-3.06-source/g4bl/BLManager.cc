...
void BLManager::appendVerboseFormat(G4String fmt)
{
	...
	else if(f == "CHARGE") verboseFormat.push_back(CHARGE);
	else if(f == "OCUP") verboseFormat.push_back(OCUP);
...
clx:		    if(header) {
		...
		case CHARGE:
					...
					printf(" %8.3f",track->GetDynamicParticle()->GetCharge());
				    }
					break;
		case OCUP:
						    if(header) {
							...
							printf(" %8.3f",track->GetDynamicParticle()->GetTotalOccupancy());
						    }
							break;
		case SEG:
		    if(header) {
