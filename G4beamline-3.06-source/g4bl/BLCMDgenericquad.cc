...
void BLCMDgenericquad::defineNamedArgs()
{
	...
	argDouble(coilHalfwidth,"coilHalfWidth","Synonym for coilHalfwidth.");
	 argDouble(gradientM,"gradientM","The magnetic field gradient, dBy/dx (Tesla/meter)",tesla/meter);
	
	// Electrostatic edit:
	argDouble(gradientE,"gradientE","The Electrostatic field gradient (dEy/dx (megavolt/meter))",megavolt/meter);
	...
	BLCMDgenericquad *quad) : BLElementField(), rotation(), enge(ENGE_QUAD)
{
	...
	gradientM = quad->gradientM;
	gradientE = quad->gradientE;
								...
								const
{
	...
	G4double G0M = gradientM;
	G4double G0E = gradientE;
	double fringeZ = (fabs(local[2])-halflength)/fringeDepth;
	G4double f = enge(fringeZ);
	G4double fp = enge.prime(fringeZ)/fringeDepth;
	G4ThreeVector B(G0M*f*local[1],G0M*f*local[0],G0M*fp*local[0]*local[1]);
	G4ThreeVector E(-G0E*f*local[0],G0E*f*local[1],G0E*fp*local[0]*local[1]);
	if(local[2] < 0.0) B[2] = -B[2];
	if(local[2] < 0.0) E[2] = -E[2];
