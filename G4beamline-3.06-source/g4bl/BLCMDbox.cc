...
public:
	...
	// GDMLparser
	   ...
	   // Reading and Writing Settings
	     //
	     G4String fReadFile;
	     G4VSolid* Temp_volume1;
         G4LogicalVolume* Temp_volume;
         G4LogicalVolume *lv;
	/// isOK() returns true.
...
void BLCMDbox::defineNamedArgs()
{
	...
	argString(fReadFile,"fReadFile","Write name of the gdml file if replacement wanted or leave empty (''=DNA stronger.gdml).");
			...
			G4ThreeVector parentPosition)
{
	...
	Temp_volume=fParser.GetWorldVolume()->GetLogicalVolume();//->GetDaughter(0)->GetLogicalVolume()->GetSolid();
    lv = Temp_volume;
   	fParser.Clear();
	}else{Temp_volume1=box;
        lv = new G4LogicalVolume(Temp_volume1,mat, thisname+"LogVol");
    }

	//G4LogicalVolume *lv = new G4LogicalVolume(Temp_volume,mat, thisname+"LogVol");
    //G4LogicalVolume *lv = Temp_volume;
